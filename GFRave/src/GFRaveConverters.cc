...
GFTrackToTrack(trackAndState trackAndState, int id, std::string tag){
  ...
  //std::cerr<<"  pos: "; Point3DToROOT::Math::XYVector(ravestate.position()).Print();
  //std::cerr<<"  mom: "; Vector3DToROOT::Math::XYVector(ravestate.momentum()).Print();
...
setData(const rave::Track& orig, MeasuredStateOnPlane* state){
  ...
  state->setPosMomCov(ROOT::Math::XYVector(orig.state().x(), orig.state().y(), orig.state().z()),
                      ROOT::Math::XYVector(orig.state().px(), orig.state().py(), orig.state().pz()),
...
RaveToGFVertex(const rave::Vertex & raveVertex,
  ...
  return new GFRaveVertex(Point3DToROOT::Math::XYVector(raveVertex.position()),
...
PlaneToGFDetPlane(const ravesurf::Plane& rplane) {
  return SharedPlanePtr(new DetPlane(Point3DToROOT::Math::XYVector(rplane.position()),
                    Vector3DToROOT::Math::XYVector(rplane.normalVector()) ));
}


ROOT::Math::XYVector
Point3DToROOT::Math::XYVector(const rave::Point3D& v) {
  return ROOT::Math::XYVector(v.x(), v.y(), v.z());
}

ROOT::Math::XYVector
Vector3DToROOT::Math::XYVector(const rave::Vector3D& v) {
  return ROOT::Math::XYVector(v.x(), v.y(), v.z());
}
...
rave::Point3D
ROOT::Math::XYVectorToPoint3D(const ROOT::Math::XYVector & vec){
